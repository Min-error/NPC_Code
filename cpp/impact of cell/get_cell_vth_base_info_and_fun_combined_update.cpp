#include "get_cell_vth_base_info_and_fun_update.cpp"
...
class CellInfo{
public:
    CellInfo():source_state(-1), current_state(-1), group_id(state_combined_group_num){};
    int8_t source_state;
...
struct thread_wl_info_all_p2_input{
    int wl;
    int wr;
    // int v;
    string pos1;
...
struct thread_wl_info_all_p2_retry{
    int wl;
    int wr;
    string pos1;
    string pos2;
...
void get_trans_cell_vthlist_and_avg(int wl, string pos1, string pos2, vector<vector<vector<int>>> &cell_trans_get, vector<vector<vector<int>>> &cell_trans_vthlist){
    vector<int> vth_near(window_size_l * 2 + 1, 0);
    // cout<<wl<<endl;
    for(int g = 0; g <= state_combined_group_num; g++){
        if(get_pos_wl("down", wl) <= 0 && g != state_combined_group_num) continue;
