...
struct thread_read_info{
    string in_file_path;
    vector<vector<int8_t>> *file_info;
};
...
// 文件的读入
vector<int8_t> form_state(int L, int C, int M){
    vector<int8_t> s(8, 0);
...
vector<int8_t> read_oneWL_TLC(ifstream& file){
    ...
    vector<int8_t> ret(flash_pagesize * 8, 0);
    for(int j = 0; j < flash_pagesize; ++j){
        vector<int8_t> temp = form_state(LSB[j], CSB[j], MSB[j]);
        copy(temp.begin(), temp.end(), ret.begin() + j * 8);
    }
    return ret;
}

bool read_file(string file_path, vector<vector<int8_t>> *file_info){
    ifstream default_file(file_path, ios::binary);
    if(default_file.is_open()){
        for(int i = 0; i < WLnum; i++){
            vector<int8_t> default_vec = read_oneWL_TLC(default_file);
...
void read_file_wl(int wl, string file_path, vector<int8_t> *file_info){
    ifstream default_file(file_path, ios::binary);
    streampos offset = wl * 3 * file_pagesize;
    default_file.seekg(offset, ios::beg);
    if(default_file.is_open()){
        (*file_info) = read_oneWL_TLC(default_file);
        default_file.close();
    }else{
        cout << "Read " + file_path + " Error" <<endl;
    }
...
void* read_file_thr(void *thr_read_info){
    ...
    vector<vector<int8_t>> *file_info = info.file_info;
...
void* read_file_wl_thr(void *thr_read_info){
    ...
    int wl = info.wl;
    vector<int8_t> *file_info = info.file_info;
...
// 判断wl和bl是否合法
int get_pos_wl_old(string pos, int i){
    if(pos == "down"){
        if(i - WLnum_pre_layer < 0) return -2;
        if(i >= center_wl && i - WLnum_pre_layer < center_wl) return -2;
...
int get_pos_wl(string pos, int i){
    if(pos == "up"){
        ...
        if(i + WLnum_pre_layer >= WLnum) return -2;
        if(i < center_wl && i + WLnum_pre_layer >= center_wl) return -2;
        return i + WLnum_pre_layer;
    }else{
        cout<<"Error pos!" << endl;
...
// 把状态迁移表改为电压分布图
void get_trans_cell_vthlist_and_avg(string pos, vector<vector<vector<int>>> &cell_trans_get, vector<vector<vector<int>>> &cell_trans_vthlist, int all_group_num = state_group_num){
    ...
    for(int g = 0; g < all_group_num; g++){
        for(int j = 0; j < state_group_num * state_group_num; j++){
            for(int v = 1; v < window_size_l * 2 + 1; v++){
                vth_near[v] = abs(cell_trans_get[g][v][j] - cell_trans_get[g][v - 1][j]);
            }
            for(int v = 1 + window_size_l; v < (Sread - 1) * 2 + 1 - window_size_l; v++){
                ...
                vth_near[window_size_l * 2] = abs(cell_trans_get[g][v + window_size_l][j] - cell_trans_get[g][v + window_size_l - 1][j]);

                int sum_vth_near = 0;
                for(int k = 0; k < window_size_l * 2 + 1; k++) sum_vth_near += vth_near[k];
                cell_trans_vthlist[g][v][j] = static_cast<int>((sum_vth_near) / (window_size_l * 2.0 + 1.0) + 0.5);
            }
        }
    }
}
